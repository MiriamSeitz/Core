...
class UiPage implements UiPageInterface
{
    ...
    public function getWidget($id, WidgetInterface $parent = null)
    {
        ...
        // id space via `_`: e.g. `.DataTable` => `DataTable` + `_`.
        // The check for the length of the id space works because just `DataTable` will have a length `false` and
        // would mean "no id space given", while `.DataTable` with an empty space will have a length of `0` and
     ...
     * ## Debugging
     * 
     * This method can run into infinite loops when it searches for widget_id, which is not an autogenerated path, but
     * was set by the user explicitly. In this case, it may happen, that the algorithm starts checking a branch of the
     * widget tree, which is very deep or even recursive - then it will get either really slow or go into an inifinite
     * loop. So far we cannot detect these situations reliably.
     * 
     * If you have a UI issue, when an AJAX request simply times out, it is often related to issues with this method.
     * 
     * There is a simple way to check: add a conditional breakpoint, with a condition like this: 
     * 
     * ```
     * count(explode('_', $parentIdGenerated) > 20)
     * 
     * ```
     * 
     * This will basically break if the parent widget has a big depth. If you set the threshold big enough, you will
     * be able to see the loop in the stack trace. The reasons can be very different.
     * 
     * @param string $id
     ...
     */
    private function getWidgetFromIdSpace($id, $id_space, WidgetInterface $parent, $use_id_path = true, int $maxDepth = 50)
    {
        if ($maxDepth < 0) {
        ...
        // There are two ways to reference the root id space explicitly: e.g. `.DataTable` and `_.DataTable`.
        // Normalize them to `_.DataTable` format here
        ...
        foreach ($parent->getChildren() as $child) {
            $childId = $child->getId();
            // Get the id space of the child widget if it has one (otherwise it will be in the id space of the parent,
            // so we can save some performance on the inheritance logic here)
            $childIdSpace = $child->isIdSpaceSpecified() ? $child->getIdSpace() : $parentIdSpace;
            // $childIdSpace = $child->getIdSpace();
            ...
            if ($childId === $id_with_namespace || $childIdGenerated === $id_with_namespace) {
                return $child;
            } elseif ($subjIdSpace && $childIdSpace === $subjIdSpace && ($subjIdInSpace === $childId || $subjIdInSpace === $childIdGenerated)) {
                return $child;
            } else {
